...
def prep_game_df_for_avgs(df, Add_new_week_sched):
    ...
    if Add_new_week_sched in ['yes','Yes','YES','y','Y']:
        ...
        game_df5 = game_df5.drop(['index'],axis = 1)
        #game_df4.concat(new_week_sched,how = 'outer',left_index=True,right_index=True)
        game_df4 = game_df4.append(new_week_sched)
                        ...
                        if not temp.empty:
                            ...
                            for game in temp['game_id'].unique():
                                ...
                                if team_check == team: #if team is home team
                                    ...
                                    temp2 = temp2.drop(['away_wins','away_losses','away_ties','home_winner','away_winner','home_spread','away_spread'],axis=1)
                                    dic_home = rename_cols('home',temp2)
                                    temp2.rename(columns=dic_home,inplace = True)
                                    temp2['team'] = team
                                else:
                                    temp2 = game_team.copy()
                                    temp2 = temp2.drop(['home_wins','home_losses','home_ties','home_winner','away_winner','home_spread','away_spread'],axis=1)
